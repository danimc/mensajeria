...
$("#agregarModelo").on("click", () => {
    ...
    const marca = $("#marca1").val();
    const tipo = $("#tipo1").val();
...
$("#agregarCatActivo").click(() => {
    ...
    if (categoria.length > 3) {
        ...
        $.ajax({
            ...
            success: (msg) => {
                ...
                if (!msg.error) {
                    ...
                    const tipo = new Tipo(msg.data.id_clasificacion, msg.data.clasificacion, msg.data.red, msg.data.img);
                    tipo.nuevaCategoria();
                }
            },
        });
        ...
        setTimeout('document.getElementById("agregarCatActivo").disabled=false', 1000);
    }
});
$("#tb_catActivos tbody").on("click", "button", function () {
    let data = tbTipo.row($(this).parents("tr")).data();
...
function eliminar(tipo, tabla, fila) {
    let url = ruta;
    alertify.confirm("¿Esta Seguro que desea eliminar el Item del catálogo?", () => {
        if (tabla == 1) {
            if (tipo.id > 21) {
                tipo.eliminarTipo(fila);
            }
            else {
                alertify.alert("<h2><i class='fa fa-close' style='color: red;' ></i> ¡ERROR!</h2> <br>NO SE PUEDE ELIMINAR DEL CATÁLOGO REGISTROS GENERADOS POR EL SISTEMA", function () {
        ...
        if (tabla == 2) {
            ...
            if (pedidos == 0) {
                ...
                alertify.alert("<h2><i class='fa fa-close' style='color: red;' ></i> ¡ERROR!</h2> <br>NO SE PUEDE ELIMINAR DEL CATÁLOGO CONSUMIBLES QUE SE ENCUENTREN EN VALES ACTIVOS", function () {
                    alertify.error("Acción Cancelada");
                });
            }
        }
