...
class Oficios extends CI_Controller
{
     ...
     * @return Json
     */
    public function obtNombreDependencias()
    {
        header('Content-Type: application/json');
     ...
     * @return void
     */
    public function capturarOficio()
    {

        $max = $this->m_oficios->obtMaxConsecutivo();
        $dependencia = $this->input->post('dependencia');
        $datosDependencia = $this->m_oficios->obtDatosDependencia($dependencia);
     ...
     * @return json
     */
    public function obtConsecutivo()
    {
        $consecutivo = $this->m_oficios->obtMaxConsecutivo();
     ...
     * @return Json
     */
    public function obtDatosDependencia()
    {
     ...
     * @return void
     */
    public function subir_oficio($id, $consecutivo, $year)
    {
     ...
     * @return void
     */
    public function subeOriginal($id, $consecutivo, $year)
    {
        // SCRIPT PARA SUBIR LOS PDF ###########################
        if ($_FILES['pdf']['name'] != "") {
            $ext = explode('.', $_FILES['pdf']['name']);
     ...
     */
    public function obtHistorialOficio()
    {
        $id = $this->input->get('id');
